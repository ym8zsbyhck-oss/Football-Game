...
window.MatchEngine = class {
  ...
  teamNames(team){
    const h=this.home?.id||"", a=this.away?.id||"";
    const map={
  ...
  start(home,away,userSide){
    ...
    this.score=[0,0]; this.elapsed=0; this.half=1; this.halfElapsed=0; this.addedMinutes=0; this.addedReal=0;
    this.running=true; this.paused=false; this.halftimeShown=false; this.stats=this.newStats();this.events=[];
  ...
  form(side){
    ...
    const names=this.teamNames(side);
    return left.map((v,i)=>{
      let x=side?1-v[0]:v[0];
      return {
        team:side,role:roles[i],x:this.W*x,y:this.H*v[1],
        homeX:this.W*x,homeY:this.H*v[1],vx:0,vy:0,
        r:i===0?16:14,num:nums[i],speed:i===0?94:108+(i%2)*4,
        accel:i===0?560:670,hasBall:false,decision:.2+Math.random()*.3,
        stamina:1,sent:false,contactCd:0,strength:.88+((i*11)%12)/100,
        name:names[i]||["Вратарь","Защитник","Полузащитник","Плеймейкер","Нападающий"][i],
        anim:Math.random()*Math.PI*2,lastTouchTime:0
      }
    });
  }
  ...
  kickoff(team){
    ...
    this.ball={x:this.W/2,y:this.H/2,vx:0,vy:0,r:6,owner:null,lastTeam:team,state:"dead",pickupLock:0,ignorePlayer:-1,lastPassTeam:-1,lastPassTarget:-1};
  ...
  claim(i){
    ...
    if(speed>340&&Math.random()>(.48+(rating-70)/55)){this.ball.vx*=.58;this.ball.vy*=.58;return false}
    ...
    this.ball.owner=i;p.hasBall=true;this.ball.vx=this.ball.vy=0;this.ball.state="controlled";
    this.ball.ignorePlayer=-1;p.lastTouchTime=this.elapsed;
  ...
  bestPass(i,through=false){
    const p=this.players[i],aim=this.aimDirection(p);let best=null,bs=-1e9;
    this.players.forEach((q,j)=>{
      if(j===i||q.team!==p.team||q.role==="GK"||q.sent)return;
      const dx=q.x-p.x,dy=q.y-p.y,d=Math.hypot(dx,dy)||1;
      const ux=dx/d,uy=dy/d,directional=ux*aim.x+uy*aim.y;
  ...
  releasePass(through=false){
    ...
    const charge=Math.max(.15,Math.min(1,this.passCharge)),rating=((p.team===0?this.home.rating:this.away.rating)||75);
    const err=Math.max(.004,(82-rating)*.003)*(Math.random()-.5);
    const ang=Math.atan2(dy,dx)+err,pow=(through?330:245)+(through?190:210)*charge;
  ...
  releaseShot(){
    ...
    const p=this.players[this.controlled],rating=((p.team===0?this.home.rating:this.away.rating)||75);
    const gx=p.team===0?this.W+28:-28;
    // FIFA-style shot: always targets goal. Joystick does NOT determine direction.
    // Variation depends on shooting angle, rating and pressure.
    const pressure=this.players.filter(q=>q.team!==p.team&&Math.hypot(q.x-p.x,q.y-p.y)<70).length;
    const goalHalf=this.H*.105;
    const randomAim=(Math.random()-.5)*2*goalHalf;
    let gy=this.H/2+randomAim;
    const anglePenalty=Math.min(1,Math.abs(p.y-this.H/2)/(this.H*.35));
    const spread=(82-rating)*.0035 + pressure*.012 + anglePenalty*.035;
    gy+=((Math.random()-.5)*2)*this.H*spread;
    const dx=gx-p.x,dy=gy-p.y,l=Math.hypot(dx,dy)||1;
    const charge=Math.max(.2,Math.min(1,this.shotCharge));
    this.stats[p.team].shots++;
    if(Math.abs(gy-this.H/2)<this.H*.11)this.stats[p.team].onTarget++;
    this.events.push({half:this.half,minute:this.displayMinute(),type:"Удар",team:p.team,text:`${p.name} пробил по воротам`});
    this.release(this.controlled,dx/l*(360+230*charge),dy/l*(360+230*charge),-1,false);
  }

  tackle(){
    const p=this.players[this.controlled];if(!p||p.contactCd>0)return;p.contactCd=.35;
    const owner=this.ball.owner;if(owner===null)return;const o=this.players[owner];
    if(!o||o.team===p.team)return;const d=Math.hypot(p.x-o.x,p.y-o.y);if(d>42)return;
    const rating=(p.team===0?this.home.rating:this.away.rating)||75;
    if(Math.random()<Math.max(.25,Math.min(.82,.52+(rating-75)*.008-d/110))){
  ...
  separation(p){
    ...
    for(const q of this.players){
      ...
      const dx=p.x-q.x,dy=p.y-q.y,d=Math.hypot(dx,dy),safe=p.r+q.r+(q.team===p.team?24:8);
      if(d>0&&d<safe*1.5){const w=(safe*1.5-d)/(safe*1.5);sx+=(dx/d)*w;sy+=(dy/d)*w;n++}
    }return n?{x:sx/n,y:sy/n}:{x:0,y:0};
  }

  steer(p,tx,ty,dt,mul=1){
    const sep=this.separation(p);tx+=sep.x*44;ty+=sep.y*44;
    const dx=tx-p.x,dy=ty-p.y,d=Math.hypot(dx,dy),desired=Math.min(p.speed*mul,d*4.3);
  ...
  keeperAI(p,i,dt){
    const rating=(p.team===0?this.home.rating:this.away.rating)||75,gx=p.team===0?38:this.W-38;
    ...
    if(danger)tx+=p.team===0?44:-44;
    this.steer(p,tx,ty,dt,1.13+(rating-75)*.007);
    if(this.ball.owner===null&&this.ball.state==="free"&&Math.hypot(p.x-this.ball.x,p.y-this.ball.y)<31){
      const sp=Math.hypot(this.ball.vx,this.ball.vy);
      const catchChance=Math.max(.42,Math.min(.97,.78+(rating-75)*.013-sp/1050));
      if(sp<540&&Math.random()<catchChance)this.claim(i);
      else{this.ball.vx=(p.team===0?1:-1)*(290+Math.random()*120);this.ball.vy=(Math.random()-.5)*190;this.ball.lastTeam=p.team}
    }
    if(this.ball.owner===i){
      p.decision-=dt;if(p.decision<=0){p.decision=.55;
        const mates=this.players.filter(q=>q.team===p.team&&q.role!=="GK"&&!q.sent).sort((a,b)=>p.team===0?a.x-b.x:b.x-a.x);
        const t=mates[Math.min(1,mates.length-1)],dx=t.x-p.x,dy=t.y-p.y,l=Math.hypot(dx,dy)||1;
        this.release(i,dx/l*(320+(rating-75)*2),dy/l*(320+(rating-75)*2),-1,false);
      }
    }
  }

  shapeTarget(p){
    const own=this.ball.owner!==null&&this.players[this.ball.owner]?.team===p.team,dir=p.team===0?1:-1;
    const rating=(p.team===0?this.home.rating:this.away.rating)||75;let x=p.homeX,y=p.homeY;
    if(own){
      const c=this.players[this.ball.owner],push={DEF:-90,CM:35,W:95,ST:145}[p.role]||0;
      x=c.x+dir*push;if(p.role==="W")y=p.homeY;else if(p.role==="ST")y=this.H/2;else y=p.homeY+(c.y-this.H/2)*.22;
    }else{
      const shift=(this.ball.x-this.W/2)*(.16+(rating-70)*.002);
      x=p.homeX+shift;y=this.H/2+(p.homeY-this.H/2)*.72+(this.ball.y-this.H/2)*.12;
      if(p.role==="DEF")x=p.homeX+shift*.45;if(p.role==="ST")x=p.homeX+shift*.25;
    }return{x,y};
  }
  ...
  ai(p,i,dt){
    ...
    const rating=(team===0?this.home.rating:this.away.rating)||75;
    ...
    if(own&&i===this.ball.owner){
      const dir=team===0?1:-1;this.steer(p,p.x+dir*90,p.y+(this.H/2-p.y)*.05,dt,1.02+(rating-75)*.004);
      p.decision-=dt;if(p.decision<=0){p.decision=.34+Math.random()*.35-(rating-75)*.004;
        const near=team===0?p.x>this.W*.66:p.x<this.W*.34;
        if(near&&Math.abs(p.y-this.H/2)<this.H*.30&&Math.random()<(.38+(rating-70)*.012))this.aiShoot(i);
        else if(Math.random()<(.68+(rating-70)*.008))this.aiPass(i,Math.random()<.28);
      }return;
    }

    let t=this.shapeTarget(p);
    if(!own&&p===presser){t={x:this.ball.x+(team===0?-10:10),y:this.ball.y};this.steer(p,t.x,t.y,dt,1.05+(rating-75)*.004);return}
    this.steer(p,t.x,t.y,dt,.86+(rating-75)*.003);
  }

  aiPass(i,through=false){
    const t=this.bestPass(i,through);if(!t)return;
    const p=this.players[i],dx=t.x-p.x,dy=t.y-p.y,l=Math.hypot(dx,dy)||1,rating=(p.team===0?this.home.rating:this.away.rating)||75;
    const err=Math.max(.005,(82-rating)*.003)*(Math.random()-.5),ang=Math.atan2(dy,dx)+err,pow=(through?395:320)+(rating-75)*1.8;
    this.release(i,Math.cos(ang)*pow,Math.sin(ang)*pow,t.j,true);
  }

  aiShoot(i){
    const p=this.players[i],rating=(p.team===0?this.home.rating:this.away.rating)||75,gx=p.team===0?this.W+24:-24;
    const gy=this.H/2+(Math.random()-.5)*this.H*(.16-(rating-70)*.0015);
    const dx=gx-p.x,dy=gy-p.y,l=Math.hypot(dx,dy)||1;
    this.stats[p.team].shots++;if(Math.abs(gy-this.H/2)<this.H*.11)this.stats[p.team].onTarget++;
    this.events.push({half:this.half,minute:this.displayMinute(),type:"Удар",team:p.team,text:`${p.name} пробил по воротам`});
    this.release(i,dx/l*(480+(rating-75)*2.2),dy/l*(480+(rating-75)*2.2),-1,false);
  }
  ...
  setRestart(type,team,x,y){
    ...
    this.restart={type,team,t:type==="throw"?1.05:.8};
  ...
  takeRestart(){
    ...
    if(r.type==="throw"){
      ...
      if(candidate){
        ...
        if(target){let dx=target.x-this.ball.x,dy=target.y-this.ball.y,l=Math.hypot(dx,dy)||1;
          this.ball.vx=dx/l*245;this.ball.vy=dy/l*245;this.ball.lastTeam=r.team;
        }
      }return;
    }
    let tx=r.team===0?this.ball.x+135:this.ball.x-135,ty=this.H/2+(Math.random()-.5)*this.H*.38;
  ...
  resolveContacts(){
    for(let i=0;i<this.players.length;i++){const a=this.players[i];if(a.sent)continue;
      for(let j=i+1;j<this.players.length;j++){const b=this.players[j];if(b.sent)continue;
        let dx=b.x-a.x,dy=b.y-a.y,d=Math.hypot(dx,dy),min=a.r+b.r+2;if(d>=min)continue;
        if(d<.001){dx=1;dy=.3;d=Math.hypot(dx,dy)}
        const nx=dx/d,ny=dy/d,ov=min-d;a.x-=nx*ov*.5;a.y-=ny*ov*.5;b.x+=nx*ov*.5;b.y+=ny*ov*.5;
      }
    }
  }
  ...
  determineAddedTime(){
    // 0–5 minutes; more likely when there were corners/shots/events, but can also be zero/random.
    const activity=this.events.filter(e=>e.half===this.half).length+
      this.stats[0].corners+this.stats[1].corners+
      Math.floor((this.stats[0].shots+this.stats[1].shots)/3);
    let max=Math.min(5,Math.floor(activity/3)+1);
    if(Math.random()<.18)return 0;
  ...
  halftime(){
    ...
    this.halftimeShown=true;this.paused=true;
    App.showHalftime(this);
  }
  ...
  update(dt){
    ...
    if(cp&&!cp.sent){
      ...
      if(m>.04)this.steer(cp,cp.x+this.joy.x*140,cp.y+this.joy.y*140,dt,mul);
    ...
    this.players.forEach((p,i)=>{
      ...
      p.anim+=dt*(3+Math.hypot(p.vx,p.vy)/45);
  ...
  drawBall(){
    ...
    c.fillStyle="#1c1c1c";
    for(let k=0;k<5;k++){
      const a=k*Math.PI*2/5-Math.PI/2,px=Math.cos(a)*r*.45,py=Math.sin(a)*r*.45;
      c.beginPath();c.arc(px,py,r*.18,0,Math.PI*2);c.fill();
    }
    c.beginPath();c.arc(0,0,r*.22,0,Math.PI*2);c.fill();
    c.restore();
  }
  ...
  drawPlayer(p,i){
    ...
    const squish=Math.min(.10,speed/1200);
    const bob=Math.sin(p.anim)*Math.min(1.5,speed/90);

    c.save();c.translate(p.x,p.y+bob);
    if(speed>25){
      c.globalAlpha=.18;c.fillStyle="#000";
      c.beginPath();c.ellipse(-p.vx*.045,-p.vy*.045,p.r*.85,p.r*.38,0,0,Math.PI*2);c.fill();c.globalAlpha=1;
    }
    if(i===this.controlled){
      c.beginPath();c.arc(0,0,p.r+6,0,Math.PI*2);c.strokeStyle="#fff36b";c.lineWidth=3;c.stroke();
    }
    c.scale(1+squish,1-squish);
    c.beginPath();c.arc(0,0,p.r,0,Math.PI*2);c.fillStyle=base;c.fill();c.strokeStyle="#fff8";c.stroke();

    // shirt-like stripes/detail inside circular token
    c.save();c.clip();
    if(p.role!=="GK"){
      c.globalAlpha=.20;c.fillStyle=p.team?club.color:"#fff";
      c.fillRect(-p.r,-p.r,p.r*.38,p.r*2);
      c.fillRect(p.r*.20,-p.r,p.r*.38,p.r*2);
      c.globalAlpha=1;
    }
    c.restore();

    c.fillStyle=txt;c.font="900 10px Arial";c.textAlign="center";c.textBaseline="middle";c.fillText(p.num,0,0);
    c.restore();

    // surname/name above every token, like shirt/player indicator
    c.font="700 9px Arial";c.textAlign="center";c.textBaseline="bottom";
    c.fillStyle="#fff";c.strokeStyle="#0009";c.lineWidth=3;
  ...
  draw(){
    ...
    this.players.forEach((p,i)=>this.drawPlayer(p,i));
    this.drawBall();
  ...
  loop(t){
    ...
    this.elapsed+=dt;this.halfElapsed+=dt;

    if(this.halfElapsed>=this.realHalfDuration && this.addedReal===0){
      this.addedMinutes=this.determineAddedTime();
      this.addedReal=this.addedMinutes*(this.realHalfDuration/45);
    }
